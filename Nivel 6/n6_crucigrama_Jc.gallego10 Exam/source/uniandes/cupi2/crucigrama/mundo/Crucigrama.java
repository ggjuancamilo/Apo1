...
public class Crucigrama
{
	...
	/**
	 * lista que guarda la coordenada de las filas que tienen dos palabras
	 */
	public ArrayList<Integer> filasConDosPalabras;
    //-----------------------------------------------------------------
    // Constructores
    //-----------------------------------------------------------------
	...
	/**
	 * retorna la columna que tiene mas ocurrencias de la letra dada.
	 * @param pLetra letra que el usuario ingresa
	 * @returncolumna que tiene mas ocurrencias de la letra dada
	 * @throws Exception si no existe tal columna
	 */
	public int contarOcurrenciasLetraColumna( char pLetra) throws Exception
	{
		int columna = -1;
		int contador = 0;
		for (int i = 0; i < crucigrama[0].length; i++) 
		{
			...
			{
				if (crucigrama[i][j].darValIngr() == pLetra)
				{
					contador ++ ;
					if(j == crucigrama.length)
					{
						columna = contador;
						contador = 0;
					}
		...
		if (columna == -1) 
		{
			throw new Exception("No existen columnas con esa letra");
		}
		return columna;
	}
	
	/**
	 * llena la lista con las cocoordanada de las filas que tienen dos palabras
	 */
	public void filasConDosPalabras()
	{
		
		for (int i = 0; i < coordHori.length-1; i++) 
		{
			if (coordHori[i][0] == coordHori[i+1][0]) 
			{
				filasConDosPalabras.add(i);
			}
		}
	}
	/**
	 * devuelve una lista con el numero coordanada de las filas que tienen dos palabras 
	 * @return filas con dos palabras.
	 */
	public ArrayList<Integer> darfilascondospalabras()
	{
		return darfilascondospalabras();
	}
